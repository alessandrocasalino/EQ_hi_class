...
//           }
    ...
    if (strcmp(string1,"quintessence_extended") == 0) {
        pba->gravity_model_smg = quintessence_extended;
        pba->field_evolution_smg = _TRUE_;
        ...
        class_read_list_of_doubles("parameters_smg",pba->parameters_smg,pba->parameters_size_smg);
          ...
          double P_ini = pow(phi_ini_smg, alpha);  // V=cte*P(phi)
        ...
        if (has_dxdy_guess_smg == _FALSE_){
          ...
          if(pba->tuning_index_smg == 1){
             V0 = pba->Omega0_smg/pow(phi_end_guess,alpha);
             pba->tuning_dxdy_guess_smg = 1./pow(phi_end_guess,alpha);
             pba->parameters_smg[1] = V0;
          }

          if(pba->tuning_index_smg == 4){
             phi_ini_smg = pow(pba->Omega0_smg/V0, 1./alpha);
        ...
        if (strcmp(string1,"quintessence_extended_symmetron") == 0) {
            pba->gravity_model_smg = quintessence_extended_symmetron;
            pba->field_evolution_smg = _TRUE_;
            pba->is_quintessence_smg = _FALSE_;
            flag2=_TRUE_;
...
if (strcmp(string1,"nkgb") == 0 || strcmp(string1,"n-kgb") == 0 || strcmp(string1,"N-KGB") == 0 || strcmp(string1,"nKGB") == 0) {
  ...
  class_test(pba->parameters_smg[2]<0.,errmsg,"In n-KGB, Rshift0>=0, or ICs for background can't be set.");
      ...
      class_test(flag2==_FALSE_,
		 errmsg,
		 "could not identify gravity_theory value, check that it is one of 'propto_omega', 'propto_scale', 'constant_alphas', 'eft_alphas_power_law', 'eft_gammas_power_law', 'eft_gammas_exponential', 'brans_dicke', 'galileon', 'nKGB', 'quintessence_monomial', 'quintessence_extended', 'quintessence_tracker', 'alpha_attractor_canonical' ...");
